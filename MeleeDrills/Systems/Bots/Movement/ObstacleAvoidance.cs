...
{
    // Obstacle avoidance via forward "whiskers": cast a few rays ahead of the bot in its travel direction;
    // if the nearest hits static geometry within the lookahead, return a world-XZ velocity steering away
    // from the hit surface (its normal), stronger the closer the hit. Zero when the path is clear.
    //
    // This is the ONE steering behavior that is NOT pure - it queries Physics. Whiskers are cast at chest
    // height so the flat ground below isn't hit. See ObstacleLayerNames for which layers count as obstacles
    // - e.g. add "Players Out of Bounds Collider" to keep bots inside the map bounds.
    public static class ObstacleAvoidance
    {
        ...
        //   "Damageable Collider" (22) - sapper-built structures (palisades, emplacements).
        //
        // Note on (22): the buildable's ROOT GameObject sits on Default (0) and carries no collider of its
        // own - the collider lives on a CHILD on layer 22. Unity layers are per-GameObject and not inherited,
        // and a raycast reports the COLLIDER's layer, so masking Default achieved nothing here.
